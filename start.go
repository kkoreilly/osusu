	...
	return &Page{
		...
		Elements: []app.UI{
			ButtonRow().ID("start-page").Buttons(
				Button().ID("start-page-sign-in").Class("secondary").Icon("login").Text("Sign In").OnClick(NavigateEvent("/signin")),
				Button().ID("start-page-sign-up").Class("primary").Icon("app_registration").Text("Sign Up").OnClick(NavigateEvent("/signup")),
			),
