		...
		Elements: []app.UI{
			app.Form().ID("meal-page-form").Class("form").OnSubmit(m.OnSubmit).Body(
				...
				app.Div().ID("meal-page-action-button-row").Class("action-button-row").Body(
					...
					app.A().ID("meal-page-entries-button").Class("action-button", "tertiary-action-button").Href("/entries").Text("View Entries"),
				),
			),
			app.Dialog().ID("meal-page-confirm-delete").Body(
				app.P().ID("meal-page-confirm-delete-text").Text("Are you sure you want to delete this meal?"),
